...
public class MLP implements FA<double[], double[]>, Evolvable
{
    ...
    public void psoRecombine(MLP last, MLP pBest, MLP gBest)
    {
        ...
        for (int i = 0; i < inputs.length; i++)
        {
            ...
            {
                firstConnectionLayer[i][j] = firstConnectionLayer[i][j] + ki * (firstConnectionLayer[i][j] - last.firstConnectionLayer[i][j]
                        + phi1 * (pBest.firstConnectionLayer[i][j] - firstConnectionLayer[i][j])
                        + phi2 * (gBest.firstConnectionLayer[i][j] - firstConnectionLayer[i][j]));
            }
        }
        ...
        for (int i = 0; i < hiddenNeurons.length; i++)
        {
            ...
            {
                secondConnectionLayer[i][j] = secondConnectionLayer[i][j] + ki * (secondConnectionLayer[i][j] - last.secondConnectionLayer[i][j]
                        + phi1 * (pBest.secondConnectionLayer[i][j] - secondConnectionLayer[i][j])
                        + phi2 * (gBest.secondConnectionLayer[i][j] - secondConnectionLayer[i][j]));
            }
        }
